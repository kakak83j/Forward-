#!/usr/bin/env python3
"""
TELEGRAM TOPIC FORWARDER BOT - BATCH GROUPING FIX
"""
...
# ============================================
# SUBJECT DETECTOR - BATCH PRIORITY FIX
# ============================================
class SubjectDetector:
    PATTERNS = [
        # BATCH - HIGHEST PRIORITY
        re.compile(r"𝗕ᴀᴛᴄʜ\s*:\s*(.+)", re.IGNORECASE | re.UNICODE),
        re.compile(r"Batch\s*:\s*(.+)", re.IGNORECASE | re.UNICODE),
        re.compile(r"📚\s*𝗕ᴀᴛᴄʜ\s*:\s*(.+)", re.IGNORECASE | re.UNICODE),
        re.compile(r"𝐁𝐚𝐭𝐜𝐡\s*:\s*(.+)", re.IGNORECASE | re.UNICODE),
        
        # TITLE - Second priority
        re.compile(r"Tɪᴛʟᴇ\s*:\s*(.+)", re.IGNORECASE | re.UNICODE),
        re.compile(r"Title\s*:\s*(.+)", re.IGNORECASE | re.UNICODE),
        re.compile(r"🎞️\s*Tɪᴛʟᴇ\s*:\s*(.+)", re.IGNORECASE | re.UNICODE),
        re.compile(r"📑\s*Tɪᴛʟᴇ\s*:\s*(.+)", re.IGNORECASE | re.UNICODE),
        re.compile(r"𝐓𝐢𝐭𝐥𝐞\s*:\s*(.+)", re.IGNORECASE | re.UNICODE),
    ]
    ...
    def detect(caption: str) -> str:
        ...
        # Priority 1: Look for BATCH
        batch_patterns = SubjectDetector.PATTERNS[:4]  # First 4 are Batch patterns
        for pattern in batch_patterns:
            ...
            if m:
                ...
                topic = topic[:50]
                Logger.info(f"✅ Using BATCH: {topic}")
                return topic

        # Priority 2: Look for TITLE (if no Batch found)
        title_patterns = SubjectDetector.PATTERNS[4:]  # Rest are Title patterns
        for pattern in title_patterns:
            ...
            if m:
                ...
                topic = re.sub(r'\s+', ' ', topic)
                topic = topic[:50]
                Logger.info(f"⚠️ Using TITLE (no batch): {topic}")
                return topic

        # Fallback
        fallback = caption[:30].strip()
        Logger.info(f"⚠️ Using FALLBACK: {fallback}")
        return fallback if fallback else "📂 Others"
...
class TopicManager:
    ...
    def get_or_create(self, group_id: int | str, subject: str) -> int:
        ...
        if subject in self.topics:
            Logger.info(f"✅ Existing topic: {subject} -> #{self.topics[subject]}")
...
class ForwarderEngine:
    ...
    def run(self, chat_id: int | str, status_msg_id: int):
        ...
        if not test.get("ok", False):
            ...
            self.tg.send_message(chat_id, "❌ Bot cannot send messages here! Make bot admin.")
            self.running = False
            return
        ...
        for msg_id in range(start, end + 1):
            ...
            # Detect topic from BATCH (priority)
            subject = SubjectDetector.detect(caption)
            Logger.info(f"📌 Msg {msg_id} -> Topic: {subject}")
            ...
            if copy.get("ok", False):
                ...
                errors += 1
                Logger.warn(f"❌ Copy failed for msg {msg_id}: {copy}")
        ...
        self.tg.edit_message(
            ...
            f"Skipped: {skipped}\n"
            f"Errors: {errors}\n\n"
            f"📂 Topics created: {len(self.topics.topics)}"
        )
...
class BotController:
    ...
    def _send_main_menu(self, chat_id: int | str):
        ...
        topics_count = len(self.engine.topics.topics)
        self.tg.send_message(
            chat_id,
            f"🤖 <b>Topic Forwarder Bot - BATCH GROUPING</b>\n\n"
            ...
            f"✅ Bot will group by <b>BATCH</b> (not Title)",
    ...
    def _do_start(self, chat_id: int | str):
        ...
        status = self.tg.send_message(chat_id, "🚀 Starting copy (grouping by BATCH)...")
...
def main():
    ...
    Logger.info("=" * 50)
    Logger.info("  Topic Forwarder Bot - BATCH GROUPING FIX")
